...
\title{Survey of Agentic Workflow and Formalization}
...
\author{Longtian Bao, Jiajun Xi, Jingyi Zhang, Zhengan Cheng, Junfei Liu \& Jinya Jiang \\
Department of Computer Science and Engineering\\
University of California San Diego\\
La Jolla, CA 92093, USA \\
\texttt{\{lobao,j5xi,jiz290,zhcheng,jul182,j9jiang\}@ucsd.edu} \\
% \And
...
\begin{abstract}
The paradigm of artificial intelligence is undergoing a foundational transition from isolated, single-prompt large language model (LLM) queries to sophisticated, multi-agent workflows. These workflows encapsulate complex reasoning by decomposing tasks, assigning specialized roles, and orchestrating iterative collaboration. However, the deployment of these agentic systems at scale has precipitated a severe crisis in serving infrastructure. Contemporary inference engines process workflow invocations as decoupled, opaque API requests, resulting in catastrophic key-value (KV) cache redundancy, pervasive memory fragmentation, and massive scheduling bottlenecks. To resolve this critical disconnect, this survey formalizes agentic workflows into a well-defined mathematical abstraction. By exposing computation nodes, data dependencies, and orchestration operators as first-class entities to the underlying hardware runtime, this abstraction transforms opaque multi-agent executions into analyzable system workloads. Based on this formalism, we systematically explore a comprehensive design space for workflow optimization. We categorize these strategies into two complementary paradigms: execution-level optimizations, which leverage static topological motifs to guide memory management and scheduling, and graph-level optimizations, which actively restructure the workflow topology to reduce computational complexity. Ultimately, this effort demonstrates that abstracting agent workflows is not merely a software engineering convenience, but an absolute prerequisite for unlocking the next generation of LLM system optimization.
\end{abstract}
...
% Papers to be submitted to COLM 2026 must be prepared according to the
...
\section*{Statement}
We used large language models to aid and polish writing. For writing assistance, the authors drafted the text and used model suggestions to improve clarity, grammar, and
flow. All texts were reviewed and edited by the authors; the models did not originate technical ideas,
